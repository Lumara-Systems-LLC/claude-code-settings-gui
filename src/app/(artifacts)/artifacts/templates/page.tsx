...
export default function TemplatesPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-2">
          <CreateMarkdownFileDialog
            ...
            dialogDescription="Author a new project-init template"
            apiPath="/api/templates"
            queryKey="templates"
            routePrefix="/artifacts/templates"
            destinationLabel={(f) => `Will be saved as ~/.claude/templates/${f}`}
          ...
          {userError ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {userTemplates.map((template) => (
                <Link
                  key={template.name}
                  href={`/artifacts/templates/${encodeURIComponent(template.name)}`}
                >
      ...
      <Dialog open={settingsMergeDialog.open} onOpenChange={(open) => !open && setSettingsMergeDialog({ open: false, content: "", message: "" })}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>{settingsMergeDialog.message}</DialogDescription>
          </DialogHeader>
      ...
      <StarterPackDialog open={starterPackOpen} onOpenChange={setStarterPackOpen} />
    </div>
  );
}
